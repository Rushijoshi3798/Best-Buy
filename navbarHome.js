...
function GoProDivs(dataDiv,dataForGoPro){
    ...
    for (let i=0; i<dataForGoPro.length; i++){
        ...
        h3tag.innerText = "$"+ spantag.innerText;
        ...
        division.append(img, name,spantag, h3tag, buttonel);
...
function DoAddCart(obj){
    ...
    cart_items.innerText = JSON.parse(localStorage.getItem("cartNum")) || 0;
    cart_items.innerText++;
    console.log(cart_items.innerText)
    localStorage.setItem("cartNum", JSON.stringify(cart_items.innerText));
